...
export function Calculator({ data, timeUnit, onCalculate }: CalculatorProps) {
  ...
  const getValues = useCallback(() => {
    const base = isCustomFish
      ? parseFloat(customBase) || 0
      : (currentFish?.base_value ?? 0);
    const time = isCustomFish
      ? parseFloat(customTime) || 1
      : (currentFish?.time_to_roe ?? 1);
    const qMul = isCustomQuality
      ? parseFloat(customQuality) || 1
      : (currentQuality?.multiplier ?? 1);
    const mMul = isCustomMutation
      ? parseFloat(customMutation) || 1
      : (currentMutation?.multiplier ?? 1);
  ...
  const previewResult = (base * qMul * mMul) / (time || 1);

  const fishOptions = [
    ...data.fish.map((f, i) => ({
      value: String(i),
      label: `${f.fish_name} <span style="color:#64748b;font-size:0.75rem;">(base ${f.base_value}, ${f.time_to_roe}h)</span>`,
    })),
    { value: 'custom', label: '✏️ Custom Fish' },
  ];
  ...
  const handleCalculate = () => {
    const { base, time, qMul, mMul } = getValues();
    const val = (base * qMul * mMul) / (time || 1);
    setResult(val);
    setFired(true);
  ...
  return (
    <div>
      {/* Panels grid — overflow:visible is essential so dropdown menus escape the grid */}
      <div className="grid md:grid-cols-2 gap-4 mb-4" style={{ overflow: 'visible' }}>
        ...
        <Panel>
          ...
          <div className="flex flex-col gap-4">
            <Dropdown
              label="Fish" icon="🐟"
              value={selectedFish} options={fishOptions} onChange={setSelectedFish}
            />
            {isCustomFish ? (
              <div className="grid grid-cols-2 gap-3">
                <AbyssInput label="Base Value" icon="💰" value={customBase} onChange={setCustomBase} min={0} />
                <AbyssInput label="Time to Roe (h)" icon="⏱️" value={customTime} onChange={setCustomTime} min={0.1} step={0.1} />
              </div>
            ) : currentFish ? (
              <div className="grid grid-cols-2 gap-3">
        ...
        <Panel>
          ...
          <div className="flex flex-col gap-4">
            <Dropdown
              label="Quality" icon="⭐"
              value={selectedQuality} options={qualityOptions} onChange={setSelectedQuality}
            />
            {isCustomQuality && (
            ...
            <Dropdown
              label="Mutation" icon="🧬"
              value={selectedMutation} options={mutationOptions} onChange={setSelectedMutation}
            />
            {isCustomMutation && (
      ...
      >
        ...
        <span className="text-slate-200 font-bold">
          {formatValue(scaleToUnit(previewResult, timeUnit))}{timeSuffix(timeUnit)}
        </span>
      </div>
      ...
      >
        ⚗️ Calculate Pond Value
      </button>

      {/* Result */}
      {result !== null && lastEntry && (
        <div
          key={lastEntry.id}
        ...
        >
          ...
          <div className="text-slate-500 text-sm mb-4">{timeSuffix(timeUnit)}</div>
